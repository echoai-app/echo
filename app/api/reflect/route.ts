...
export async function POST(req: NextRequest) {
  let body: ReflectRequest & { recalled?: RecalledMemory[] };
  try {
    body = (await req.json()) as ReflectRequest & { recalled?: RecalledMemory[] };
  } catch {
    return NextResponse.json({ error: 'Invalid JSON' }, { status: 400 });
  }

  const { message, mode, history, recalled } = body;
  if (!message?.trim()) {
    return NextResponse.json({ error: 'message is required' }, { status: 400 });
  }

  try {
    const result = await runReflection({ message, mode, history, recalled });
