...
export default function Room() {
  ...
  const micTap = () => {
    if (vs === 'saving') return;
    if (!voice.supported) { setShowText(true); return; }
    if (vs === 'speaking') {
      // tap to interrupt Echo and start talking
      handsFree.current = true; voice.cancelSpeech(); voice.startListening(); setVs('listening');
    } else if (vs === 'listening') {
      handsFree.current = false; voice.stopListening(); setVs('idle');
    } else if (vs !== 'thinking') {
      handsFree.current = true; voice.cancelSpeech(); voice.startListening(); setVs('listening');
    }
  };
